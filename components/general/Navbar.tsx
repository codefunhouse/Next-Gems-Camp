...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      <div
        className={twMerge(
          "bg-linear-to-r from-blue-primary/50 to-[#FFA70F]/25 flex justify-center items-center w-full py-3 sm:pt-[0.95rem] sm:pb-3 px-2 text-center font-medium",
          hasScrolled
            ? "from-[#44bdf5] to-[#f3ca8c]"
            : "from-blue-primary to-[#FFA70F]/25 "
        )}
      >
        <button
          onClick={() => openModal(<ApplyForm onClick={() => closeModal()} />)}
          className="no-underline relative inline-block pb-1 transition-all duration-500 hover:before:scale-x-100 before:content-[''] before:absolute before:bottom-0 before:left-1/2 before:transform before:-translate-x-1/2 before:w-0 before:h-0.5 before:bg-grey-base before:transition-all before:duration-500 before:scale-x-0 hover:before:w-full cursor-pointer"
        >
